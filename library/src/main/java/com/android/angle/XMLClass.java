...
public abstract class XMLClass {
	...
	private void getParams() {
		for (int t = 0; t < mActivity.xmlParser.getAttributeCount(); t++)
			setCommandParam(mActivity.xmlParser.getAttributeName(t).toLowerCase(),
					mActivity.xmlParser.getAttributeValue(t).toLowerCase());
	}

	private void getFields() {
		try {
			do {
				while ((mActivity.xmlParser.getEventType() != XmlPullParser.START_TAG)
						&& (mActivity.xmlParser.getEventType() == XmlPullParser.END_DOCUMENT))
					mActivity.xmlParser.next();// skip comments
				if ((mActivity.xmlParser.getEventType() != XmlPullParser.END_DOCUMENT)
						&& (mActivity.xmlParser.getEventType() != XmlPullParser.END_TAG)) {
					String fieldName = mActivity.xmlParser.getName().toLowerCase();
					setFieldDefaults(fieldName);
					for (int t = 0; t < mActivity.xmlParser.getAttributeCount(); t++)
						setFieldParam(fieldName, mActivity.xmlParser.getAttributeName(t)
								.toLowerCase(), mActivity.xmlParser.getAttributeValue(t));
