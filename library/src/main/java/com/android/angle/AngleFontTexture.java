	...
	public Bitmap create() {
		...
		if (mTextSizeX < 11) {
			...
			while ((mFont.mCharTop[mFont.mCharCount - 1] + mFont.mHeight) > (1 << mTextSizeY))
				mTextSizeY++;
			Bitmap paintBitmap = Bitmap.createBitmap((1 << mTextSizeX), (1 << mTextSizeY),
					Config.ARGB_8888);

			Canvas canvas = new Canvas(paintBitmap);

			for (int c = 0; c < mFont.mCharCount; c++) {
				canvas.drawText(new String(mFont.mCodePoints, c, 1), 0, 1, mFont.mCharX[c]
						- mFont.mCharLeft[c] + (mFont.mBorder / 2), mFont.mCharTop[c] - minTop
						+ (mFont.mBorder / 2), paint);
			}
			mBitmap = Bitmap.createBitmap(paintBitmap, 0, 0, paintBitmap.getWidth(),
					paintBitmap.getHeight());
			paintBitmap.recycle();
		}
		return mBitmap;
	}
