			...
			for (int c = 0; c < mStep1.length(); c++) {
				...
				if (lineWidth > mDisplayWidth) {
					while ((lineWidth > mDisplayWidth) && (llc > flc)) {
						while ((mStep1.charAt(llc) == ' ') && (llc > flc)) // Quita
																			// los
																			// espacios
																			// del
																			// final
				...
				if (copy || (c == mStep1.length() - 1)) {
					...
					flc = llc + 1;
					while ((mStep1.charAt(flc) == ' ') && (flc < mStep1.length()))
	...
	private int drawLine(GL10 gl, float y, int line) {
		if ((line >= 0) && (line < mLinesCount)) {
			...
			for (int c = mLineStart[line]; (c < mLineEnd[line]) && (c < mLength); c++) {
				...
				((GL11) gl).glTexParameteriv(GL11.GL_TEXTURE_2D, GL11Ext.GL_TEXTURE_CROP_RECT_OES,
						mTextureIV, 0);

				((GL11Ext) gl).glDrawTexfOES(x + mFont.mCharLeft[chr], AngleSurfaceView.roHeight
						- (y + mFont.mHeight + mFont.mLineat), mZ, chrWidth, mFont.mHeight);
