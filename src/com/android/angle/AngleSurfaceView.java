...
public class AngleSurfaceView extends SurfaceView implements
	...
	public static void invalidateHardwareBuffers(GL10 gl) {
		...
		((GL11) gl).glBindBuffer(GL11.GL_ELEMENT_ARRAY_BUFFER,
				roIndexBufferIndex);
		((GL11) gl).glBufferData(GL11.GL_ELEMENT_ARRAY_BUFFER, 6 * 2,
				CharBuffer.wrap(sIndexValues), GL11.GL_STATIC_DRAW);
	...
	// ---------------------------------------------
	/**
	 * Used by the engine to put the new objects in the childs list. Do not use
