	...
	public void setFrame(int frame) {
		if (roLayout != null) {
			if (frame < roLayout.roFrameCount) {
				roFrame = frame;
				if (mFlipHorizontal) {
					mTextureIV[0] = (roLayout.roCropLeft + roLayout.roCropWidth)
							+ ((roFrame % roLayout.mFrameColumns) * roLayout.roCropWidth);// Ucr
					mTextureIV[2] = -roLayout.roCropWidth; // Wcr
				} else {
					mTextureIV[0] = roLayout.roCropLeft
							+ ((roFrame % roLayout.mFrameColumns) * roLayout.roCropWidth);// Ucr
					mTextureIV[2] = roLayout.roCropWidth; // Wcr
				}

				if (mFlipVertical) {
					mTextureIV[1] = roLayout.roCropTop
							+ ((roFrame / roLayout.mFrameColumns) * roLayout.roCropHeight);// Vcr
					mTextureIV[3] = roLayout.roCropHeight; // Hcr
				} else {
					mTextureIV[1] = (roLayout.roCropTop + roLayout.roCropHeight)
							+ ((roFrame / roLayout.mFrameColumns) * roLayout.roCropHeight);// Vcr
					mTextureIV[3] = -roLayout.roCropHeight; // Hcr
				}
			}
			...
			if (roLayout.roTexture != null) {
				if (roLayout.roTexture.mHWTextureID > -1) {
					...
					((GL11Ext) gl)
							.glDrawTexfOES(
									mPosition.mX
											- (roLayout.getPivot(roFrame).mX * mScale.mX),
									AngleSurfaceView.roHeight
											- ((mPosition.mY - (roLayout
													.getPivot(roFrame).mY * mScale.mY)) + (roLayout.roHeight * mScale.mY)),
									0, roLayout.roWidth * mScale.mX,
									roLayout.roHeight * mScale.mY);
				} else
