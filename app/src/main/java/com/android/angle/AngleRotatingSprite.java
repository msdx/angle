...
public class AngleRotatingSprite extends AngleAbstractSprite {
	 ...
	 *            AngleSpriteLayout
	 */
	public AngleRotatingSprite(int x, int y, float alpha,
	...
	public void setFrame(int frame) {
		if (roLayout != null) {
			if (frame < roLayout.roFrameCount) {
				...
				if ((W > 0) & (H > 0)) {
					float frameLeft = (roFrame % roLayout.mFrameColumns)
							* roLayout.roCropWidth;
					float frameTop = (roFrame / roLayout.mFrameColumns)
							* roLayout.roCropHeight;

					float left = (roLayout.roCropLeft + frameLeft) / W;
					float bottom = (roLayout.roCropTop + roLayout.roCropHeight + frameTop)
							/ H;
					float right = (roLayout.roCropLeft + roLayout.roCropWidth + frameLeft)
							/ W;
	...
	public void invalidateHardwareBuffers(GL10 gl) {
		...
		((GL11) gl)
				.glBindBuffer(GL11.GL_ARRAY_BUFFER, mTextureCoordBufferIndex);
		((GL11) gl).glBufferData(GL11.GL_ARRAY_BUFFER, 8 * 4,
			...
			if (roLayout.roTexture != null) {
				if (roLayout.roTexture.mHWTextureID > -1) {
					...
					gl.glBindTexture(GL10.GL_TEXTURE_2D,
							roLayout.roTexture.mHWTextureID);
					...
					if (AngleSurfaceView.sUseHWBuffers) {
						...
						((GL11) gl).glBindBuffer(GL11.GL_ARRAY_BUFFER,
								mVertBufferIndex);
						...
						((GL11) gl).glBindBuffer(GL11.GL_ELEMENT_ARRAY_BUFFER,
								AngleSurfaceView.roIndexBufferIndex);
						((GL11) gl).glDrawElements(GL10.GL_TRIANGLES, 6,
						...
						mIndexBuffer = ByteBuffer
								.allocateDirect(
										AngleSurfaceView.sIndexValues.length * 2)
								.order(ByteOrder.nativeOrder()).asCharBuffer();
						mVertexBuffer = ByteBuffer
								.allocateDirect(mVertexValues.length * 4)
								.order(ByteOrder.nativeOrder()).asFloatBuffer();
						mTexCoordBuffer = ByteBuffer
								.allocateDirect(mTexCoordValues.length * 4)
								.order(ByteOrder.nativeOrder()).asFloatBuffer();

						for (int i = 0; i < AngleSurfaceView.sIndexValues.length; ++i)
							mIndexBuffer.put(i,
									AngleSurfaceView.sIndexValues[i]);
						...
						gl.glDrawElements(GL10.GL_TRIANGLES,
								AngleSurfaceView.sIndexValues.length,
