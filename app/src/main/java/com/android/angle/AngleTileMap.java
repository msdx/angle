...
public class AngleTileMap extends AngleObject {
	...
	public AngleTileMap(AngleTileBank tileBank, int width, int height,
			int columnsCount, int rowsCount, boolean horizontalRepeat,
			...
			if (mTileBank.mTexture.mHWTextureID > -1) {
				if (mTileBank != null) {
					...
					gl.glBindTexture(GL10.GL_TEXTURE_2D,
							mTileBank.mTexture.mHWTextureID);
					gl.glColor4f(mRed, mGreen, mBlue, mAlpha);

					for (int y = 0; y < ttdY; y++) {
						int py = y * TH - (((int) (mTop * mScale)) % TH);

						mTextureIV[3] = -mTileBank.mTileHeight;// (int)
																// (-H/mScale);
																// // Hcr
						for (int x = 0; x < ttdX; x++) {
							int tile = getTile(x + (int) (mLeft * mScale / TW),
									y + (int) (mTop * mScale / TH));
							int px = x * TW - (((int) (mLeft * mScale)) % TW);

							if ((tile >= 0) && (tile < mTileBank.mTilesCount)) {
								mTextureIV[0] = (tile % mTileBank.mTilesColumns)
										* (mTileBank.mTileWidth);// +(dx/mScale));//
																	// Ucr
								mTextureIV[1] = (tile / mTileBank.mTilesColumns)
										* (mTileBank.mTileHeight)
										+ mTileBank.mTileHeight;// -(dy/mScale));//
																// Vcr
								mTextureIV[2] = mTileBank.mTileWidth;// (int)
																		// (W/mScale);
																		// //
																		// Wcr
								((GL11) gl).glTexParameteriv(
										...
										mTextureIV, 0);
								((GL11Ext) gl).glDrawTexfOES(mPosition.mX + px,
										AngleSurfaceView.roHeight
												- (mPosition.mY + py + TH), mZ,
										TW, TH);
							}
						}
					}
