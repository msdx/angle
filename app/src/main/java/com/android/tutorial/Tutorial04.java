...
/**
 * Cretate an user interface (AngleUI) >Creaci�n de una interface de usuario
 * (AngleUI)
 * 
 * @author Ivan Pajuelo
 * 
...
public class Tutorial04 extends AngleActivity {
	...
	private class MyDemo extends AngleUI {
		...
		// >Ahora nuestro(s) sprite(s) giratorio(s) estar�(n) en nuestra nueva
		// UI
		...
		public boolean onTouchEvent(MotionEvent event) {
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				...
				// >A�adimos un nuevo MyAnimatedSprite en la posici�n donde se
				// ha pulsado
				// >Para acceder a mGLSurfaceView usaremos mActivity
				mActivity.mGLSurfaceView.addObject(new MyAnimatedSprite((int) event.getX(),
						(int) event.getY(), mLogoLayout));
				return true;
			}
