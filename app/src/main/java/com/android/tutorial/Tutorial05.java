...
/**
 * Use some pseudo-phisyc >Usando algo de pseudo-f�sica
 * 
 * 
 * @author Ivan Pajuelo
 * 
...
public class Tutorial05 extends AngleActivity {
	...
	private class MyDemo extends AngleUI {
		...
		public MyDemo(AngleActivity activity) {
			super(activity);
			mBallLayout = new AngleSpriteLayout(mGLSurfaceView, 64, 64, R.drawable.ball, 0, 0, 128,
					128);
		...
		public boolean onTouchEvent(MotionEvent event) {
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				if ((event.getX() > 30) && (event.getY() > 30) && (event.getX() < 320 - 30)
						&& (event.getY() < 480 - 30)) {
