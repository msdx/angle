...
/**
 * Cretate little game simulation >Creamos una peque�a simulaci�n de juego
 * 
 * @author Ivan Pajuelo
 * 
 */
public class Game extends AngleActivity {
	...
	public class MyDemo extends AngleUI {
		...
		public MyDemo(AngleActivity activity) {
			...
			// Create and insert two main object groups. One for the game field
			// and other for the dashboard
			// >Creamos e insertamos dos grupos de objetos principales. Uno para
			// el campo de juego y otro para los marcadores
			ogField = addObject(new AngleObject());
			ogDashboard = addObject(new AngleObject());

			// Create a tile bank and a tile map for the ground (see
			// constructors)
			// >Creamos un banco de tiles y un mapa para el suelo (mirar los
			// constructores)
			AngleTileBank tbGround = new AngleTileBank(mActivity.mGLSurfaceView,
					R.drawable.tilemap, 4, 4, 32, 32);
			...
			slShip = new AngleSpriteLayout(mActivity.mGLSurfaceView, 64, 64, R.drawable.anglelogo,
					0, 0, 128, 128);
			slShot = new AngleSpriteLayout(mActivity.mGLSurfaceView, 16, 16, R.drawable.anglelogo,
					0, 0, 128, 128);
			mShip = (MyShip) ogField.addObject(new MyShip(slShip));

			// The dashboard background
			// >Fondo de los marcadores
			AngleSpriteLayout slDash = new AngleSpriteLayout(mActivity.mGLSurfaceView, 320, 64,
					R.drawable.tilemap, 0, 32, 320, 64);
			...
			// Font and text
			// >Fuente y texto
			AngleFont fntCafe25 = new AngleFont(mActivity.mGLSurfaceView, 25,
					Typeface.createFromAsset(getAssets(), "cafe.ttf"), 222, 0, 0, 30, 200, 255, 255);
			ogDashboard.addObject(new AngleString(fntCafe25, "Hello!!!", 160, 440,
		...
		public boolean onTouchEvent(MotionEvent event) {
			...
			// -------------------------
			mShip.mDestination.set(event.getX(), event.getY() - 32 - slShip.roHeight / 2); // Position
																							// the
																							// ship
			long CTM = SystemClock.uptimeMillis();
